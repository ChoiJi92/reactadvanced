...
const Write = () => {
  ...
  return (
    <Container>
      ...
      <Ridio check={layout}>
          <input type='radio' id = 'right' value='right' checked={layout === 'right' }readOnly
          onClick={() => setLayout('right')}></input>
          <label htmlFor="right" onClick={() => setLayout('right')}>오른쪽에 이미지 왼쪽에 텍스트</label>
      </Ridio>
      <Layout>
        <p>{posting}</p>
        <img
      ...
      <Ridio check={layout}>
          <input type='radio' id = 'left' value='left' checked={layout === 'left'}readOnly
          onClick={() => setLayout('left')}></input>
          <label htmlFor="left" onClick={() => setLayout('left')}>왼쪽에 이미지 오른쪽에 텍스트</label>
      </Ridio>
      <Layout>
        <img
          src={image}
      ...
      <Ridio check={layout}>
          <input type='radio' id = 'bottom' value='bottom' checked={layout === 'bottom'}readOnly
          onClick={() => setLayout('bottom')}></input>
          <label htmlFor="bottom" onClick={() => setLayout('bottom')}>하단에 이미지 상단에 텍스트</label>
      </Ridio>
      <Layoutbottom>
        <p>{posting}</p>
        <img
      ...
      <Button disabled={!image || !layout || !posting ? true : false } onClick={addcomment}>게시글 작성</Button>
    </Container>
  );
};
